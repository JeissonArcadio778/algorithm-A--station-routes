...
def determine_optimal_route(origen, destination, knowledge_base):
    ...
    while open_nodes:
        ...
        current_node = min(
            open_nodes,
            key=lambda x: x[1] + calculate_heuristic(x[0], destination)
            )
...
optimal_route = determine_optimal_route(
                start_station,
                destination,
                knowledge_base
                )
...
if optimal_route:
    print("La mejor ruta desde {} hasta {} es:".format(start_station, destination)
          + "\n" +
          "->".join(optimal_route))
else:
    print("No hay una ruta {} hasta {}".format(start_station, destination))
